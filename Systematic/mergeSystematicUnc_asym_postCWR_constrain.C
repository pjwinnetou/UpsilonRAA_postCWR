...
void mergeSixInQuad( TH1D* h0=0, TH1D* h1=0, TH1D* h2=0, TH1D* h3=0, TH1D* h4=0, TH1D*h5=0, TH1D*h6=0, int state=1, TString title="", TString Asym="");
...
void mergeSystematicUnc_asym_postCWR_constrain(int state = 1, TString Asym="Hi") { 
  ...
  //TFile* f2 = new TFile(Form("../compareDataMc/20170405_AccCleaned/20170106/CompAcc_20170112/sys_acceptance_ups%dS_draft.root",state));
  TFile* f2 = new TFile(Form("../acceptance/sys_acceptance_ups%dS_1804.root",state));
  ...
  for ( int ibin = 1 ; ibin <= hcentAA[2]->GetNbinsX() ; ibin++ ) {
    hcentAA[2]->SetBinContent(ibin, 0.00);
  }
  ...
  // 3 : signal Par
  TFile* f3 = new TFile(Form("SignalVariation/Constrain_signalPar_%ds.root",state));
  ...
  // 4 : background PDF 
  TFile* f4 = new TFile(Form("BkgVariation_PostCWR/Constrain_bkgPDFVariaion_4th_%ds.root",state));
  TFile* f4_1 = new TFile(Form("BkgVariation_PostCWR/Constrain_bkgPDFVariaion_linear_%ds.root",state));
...
*/
  ...
  // 5 : CB+Gaus PDF 
  TFile* f5 = new TFile(Form("SignalVariation_PostCWR/Constrain_signalPDFVariaion_%ds.root",state));
  ...
  cout << "TAA : " << hptRAA[6]->GetBinContent(1) << endl;
  cout << "TAA : " << hptRAA[6]->GetBinContent(2) << endl;

  mergeSixInQuad( hptPP[0], hptPP[1], hptPP[2], hptPP[3], hptPP[4], hptPP[5], hptPP[6], state, "pp Unc. vs p_{T}" ,Asym);
  mergeSixInQuad( hrapPP[0], hrapPP[1], hrapPP[2], hrapPP[3], hrapPP[4], hrapPP[5], hrapPP[6], state, "pp Unc. vs y",Asym );
  mergeSixInQuad( hptAA[0], hptAA[1], hptAA[2], hptAA[3], hptAA[4], hptAA[5], hptAA[6], state, "PbPb Unc. vs p_{T}",Asym );
  mergeSixInQuad( hrapAA[0], hrapAA[1], hrapAA[2], hrapAA[3], hrapAA[4], hrapAA[5], hrapAA[6] , state, "PbPb Unc. vs y",Asym);
  mergeSixInQuad( hcentAA[0], hcentAA[1], hcentAA[2], hcentAA[3], hcentAA[4], hcentAA[5], hcentAA[6] , state, "PbPb Unc. vs Centrality",Asym);
  mergeSixInQuad( hintAA[0], hintAA[1], hintAA[2], hintAA[3], hintAA[4], hintAA[5], hintAA[6] , state,"AA Unc. integrated",Asym);
  mergeSixInQuad( hintPP[0], hintPP[1], hintPP[2], hintPP[3], hintPP[4], hintPP[5], hintPP[6] , state,"PP Unc. integrated",Asym);

  mergeSixInQuad( hptRAA[0], hptRAA[1], hptRAA[2], hptRAA[3], hptRAA[4], hptRAA[5], hptRAA[6] , state,"RAA Unc. vs p_{T}", Asym);
  mergeSixInQuad( hrapRAA[0], hrapRAA[1], hrapRAA[2], hrapRAA[3], hrapRAA[4], hrapRAA[5], hrapRAA[6] , state,"RAA Unc. vs |y|", Asym);
  mergeSixInQuad( hcentRAA[0], hcentRAA[1], hcentRAA[2], hcentRAA[3], hcentRAA[4], hcentRAA[5], hcentRAA[6] , state, "RAA Unc. vs cent.",Asym);
  mergeSixInQuad( hintRAA[0], hintRAA[1], hintRAA[2], hintRAA[3], hintRAA[4], hintRAA[5], hintRAA[6] , state,"RAA Unc. int.",Asym);
  ...
  */
  ...
  TFile* fout = new TFile(Form("postCWR_mergedSys_constrain_ups%ds_asym%s.root",state,Asym.Data()),"recreate" );
  hptPP[3]->Write();
  hptAA[3]->Write();
  hrapPP[3]->Write();
  hrapAA[3]->Write();
  ...
  TFile* fout_sys = new TFile(Form("PDFSystematic_postCWR_constrain_%ds.root",state),"recreate");
  fout_sys->cd();
  hptPP[4]->Write();
  hptAA[4]->Write();
  hrapPP[4]->Write();
...
void mergeSixInQuad( TH1D* h0, TH1D* h1, TH1D* h2, TH1D* h3, TH1D* h4, TH1D* h5, TH1D* h6, int state, TString title, TString Asym) {
...
//  mergeTwoInQuad( hsigMerged, h3, h5);
  ...
  if(state==1) h0->SetAxisRange(0,0.4,"Y");
  else if(state==1) h0->SetAxisRange(0,0.7,"Y");
  ...
  handsomeTH1(h5,        7); h5->SetLineWidth(2); h5->DrawCopy("hist same");
  handsomeTH1(h6,11);         h6->SetLineWidth(2); h6->DrawCopy("hist same");
  ...
  leg1->Draw();
  c0->SaveAs(Form("pdfFiles/All_postCWR_%s_ups%ds_%s.pdf", h0->GetName(),state,Asym.Data() ) );
  // 6 : TAA uncertainty
  // 5 : CB+Gaus PDF  
  // 4 : background PDF
  // 3 : signal Par
...
void mergeFiveInQuad( TH1D* h0, TH1D* h1, TH1D* h2, TH1D *h3, TH1D* h4, TH1D* h5, int state, TString title) {
  ...
  c0->SaveAs(Form("pdfFiles/postCWR_free_%s_ups%ds.pdf", h0->GetName(),state ) );
