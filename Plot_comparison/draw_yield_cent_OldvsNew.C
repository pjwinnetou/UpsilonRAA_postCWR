...
int draw_yield_cent_OldvsNew(int states =1, TString ColId = "AA")
{
  /////////////////////////////////////////////////////////
  //// set style
  /////////////////////////////////////////////////////////
  ...
  for (int ib =0; ib < nBin; ib ++ ) {
   ...
   if(ColId=="AA"){
   fileIn1[ib]= new TFile(Form("../../../upsilonRAA5TeV/fitResults/Final_NomResult_170124/PAS_fitresults_upsilon_DoubleCB_AA_DATA_pt0.0-30.0_y0.0-2.4_muPt4.0_centrality%.0f-%.0f_dphiEp_0.00PI_100.00PI.root",binArr[ib],binArr[ib+1]));
   fileIn2[ib]= new TFile(Form("../fitResults/FixedFit_n_alpha_x_Avg_frompp/fitresults_upsilon_fixParm1_seed2_DoubleCB_AA_DATA_pt0.0-30.0_y0.0-2.4_muPt4.0_centrality%.0f-%.0f_dphiEp_0.00PI_100.00PI.root",binArr[ib],binArr[ib+1]));}
   else if(ColId=="PP"){
   fileIn1[ib]= new TFile(Form("../../../upsilonRAA5TeV/fitResults/Final_NomResult_170124/PAS_fitresults_upsilon_DoubleCB_AA_DATA_pt0.0-30.0_y0.0-2.4_muPt4.0_centrality%.0f-%.0f_dphiEp_0.00PI_100.00PI.root",binArr[ib],binArr[ib+1]));
   fileIn2[ib]= new TFile(Form("../fitResults/FixedFit_n_alpha_x_Avg/fitresults_upsilon_fixParm1_seed2_DoubleCB_AA_DATA_pt0.0-30.0_y0.0-2.4_muPt4.0_centrality%.0f-%.0f_dphiEp_0.00PI_100.00PI.root",binArr[ib],binArr[ib+1]));}
  ...
  //// histogram
  TH1D* h1_nSig1s = new TH1D("h1_nSig1s","h1_nSig1s;Centrality;events",nBin,binArr); 
  TH1D* h1_nSig2s = new TH1D("h1_nSig2s","h1_nSig2s;Centrality;events",nBin,binArr); 
  TH1D* h1_nSig3s = new TH1D("h1_nSig3s","h1_nSig3s;Centrality;events",nBin,binArr); 
  TH1D* h1_nBkg = new TH1D("h1_nBkg","h1_nBkg;Centrality;events",nBin,binArr); 
  
  TH1D* h2_nSig1s = new TH1D("h2_nSig1s","h2_nSig1s;Centrality;events",nBin,binArr); 
  TH1D* h2_nSig2s = new TH1D("h2_nSig2s","h2_nSig2s;Centrality;events",nBin,binArr); 
  TH1D* h2_nSig3s = new TH1D("h2_nSig3s","h2_nSig3s;Centrality;events",nBin,binArr); 
  TH1D* h2_nBkg = new TH1D("h2_nBkg","h2_nBkg;Centrality;events",nBin,binArr); 

  TFile *fout=new TFile(Form("yield/Yields_cent_%ds.root", states),"recreate");
  ...
  cout<< "nSig1S(7): " << h1_nSig1s->GetBinContent(7) <<endl;
  cout<< "nSig1S(8): " << h1_nSig1s->GetBinContent(8) <<endl;
  cout<< "nSig1S(9): " << h1_nSig1s->GetBinContent(9) <<endl;
  ...
  cout<< "nSig2S(4): " << h1_nSig2s->GetBinContent(4) <<endl;
  cout<< "nSig2S(5): " << h1_nSig2s->GetBinContent(5) <<endl;
  cout<< "nSig2S(6): " << h1_nSig2s->GetBinContent(6) <<endl;
  cout<< "nSig2S(7): " << h1_nSig2s->GetBinContent(7) <<endl;
  cout<< "nSig2S(8): " << h1_nSig2s->GetBinContent(8) <<endl;
  cout<< "nSig2S(9): " << h1_nSig2s->GetBinContent(9) <<endl;
...
//  gPad->SetLogy(1);  // KTO for yield
  ...
  if(ColId == "PP") h1_nSig1s->GetYaxis()->SetRangeUser(1,8000);
  if(ColId == "AA") h1_nSig1s->GetYaxis()->SetRangeUser(1,1800);
  ...
  c_nSig1s->SaveAs(Form("yield/cent_nSig1s_%s_%dS.pdf",ColId.Data(),states));
...
//  gPad->SetLogy(1);  // KTO for yield
  ...
  if(ColId == "PP") h1_nSig2s->GetYaxis()->SetRangeUser(1,8000);
  if(ColId == "AA") h1_nSig2s->GetYaxis()->SetRangeUser(1,210);
  if(ColId == "AA") h2_nSig2s->GetYaxis()->SetRangeUser(1,210);
  ...
  latex->SetTextColor(kBlue+2);
  latex->DrawLatex(0.55,0.73,Form("(Old) #Upsilon(%dS) yields",states));
  c_nSig2s->SaveAs(Form("yield/cent_nSig2s_%s_%dS.pdf",ColId.Data(),states));
...
//  h1_nSig3s->GetYaxis()->SetRangeUser(1,1000);
  ...
  c_nSig3s->SaveAs(Form("yield/cent_nSig3s_%s_%dS.pdf",szAA.Data(),states));
...
//  gPad->SetLogy(1);  // KTO for yield
  ...
  c_nBkg->SaveAs(Form("yield/cent_nBkg_%s_%dS.pdf",szAA.Data(),states));
*/
