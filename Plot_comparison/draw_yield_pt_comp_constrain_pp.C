...
int draw_yield_pt_comp_constrain_pp(TString szAA = "PP", int states =1) 
{
  setTDRStyle();
...
*/
  ...
  char *Fit_loc[3] = {"../../../upsilonRAA5TeV/fitResults/Final_NomResult_170124/","../fitResults/FixedFit_n_alpha_x_Avg_frompp/", "../fitResults/Constrain/"};
  char *Name_Fit[3] = {"All Free","n, #alpha, x, f fixed", "n, #alpha, x, f constrained"};
  ...
  if(szAA=="PP"){fileLoc[1] = "../fitResults/FixedFit_n_alpha_x_Avg/"; fitName[1] = "n, #alpha, x fixed"; fitName[2] = "n, #alpha, x constrained";}
  ...
  gSig[0]->GetYaxis()->SetTitle("dN/dp_{T}");
  gSig[0]->GetXaxis()->SetTitle("p^{#varUpsilon}_{T} (GeV/c)");
  gSig[0]->GetYaxis()->CenterTitle();
  gSig[0]->GetXaxis()->CenterTitle();
...
//  gPad->SetLogy();  // KTO for yield
  for(int ifit=0; ifit<nFit; ifit++){ 
    if(ifit==0) gSig[ifit]->Draw("AP");
    else gSig[ifit]->Draw("P");
  }
  ...
  c_nSigs->SaveAs(Form("yield/pt_nSig%ds_%s_constrain.pdf",states,szAA.Data()));
  ...
  c_nBkg->cd();
//  gPad->SetLogy();  // KTO for yield
  for(int ifit=0; ifit<nFit; ifit++){ if(ifit==0) gBkg[ifit]->Draw("AP"); else gBkg[ifit]->Draw("P");   }
  latex->SetTextColor(kBlack);
  ...
  c_nBkg->SaveAs(Form("yield/pt_nBkg%ds_%s_constrain.pdf",states,szAA.Data()));
