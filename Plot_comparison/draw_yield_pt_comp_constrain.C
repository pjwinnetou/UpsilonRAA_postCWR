...
int draw_yield_pt_comp_constrain(TString szAA = "AA", int states =1) 
{
  setTDRStyle();
...
*/
  ...
  char *Fit_loc[2] = {"../fitResults/FixedFit_n_alpha_x_Avg_frompp/", "../fitResults/Constrain/"};
  char *Name_Fit[2] = {"n, #alpha, x, f fixed", "n, #alpha, x fixed, f constrained"};
  ...
  if(szAA=="PP"){fileLoc[0] = "../fitResults/FixedFit_n_alpha_x_Avg/"; fitName[0] = "n, #alpha, x fixed"; fitName[1] = "n, #alpha, x constrained";}
  ...
  for (int ib =0; ib < nBin; ib ++ ) {
    for(int ifit = 0; ifit < nFit; ifit++){
      //// read files
      if (szAA == "PP" ) { 
        if(ifit==0) {fileIn[ifit][ib]= new TFile(Form("%sfitresults_upsilon_fixParm1_seed2_DoubleCB_%s_DATA_pt%.1f-%.1f_y0.0-2.4_muPt4.0.root",fileLoc[ifit].Data(),szAA.Data(),binArr[ib],binArr[ib+1]));}
        else if(ifit==1) {fileIn[ifit][ib]= new TFile(Form("%sfitresults_upsilon_fixParm1_seed2_DoubleCB_%s_DATA_pt%.1f-%.1f_y0.0-2.4_muPt4.0.root",fileLoc[ifit].Data(),szAA.Data(),binArr[ib],binArr[ib+1]));}
      }
      else if (szAA == "AA" ) { 
        if(ifit==0) {fileIn[ifit][ib]= new TFile(Form("%sfitresults_upsilon_fixParm1_seed2_DoubleCB_%s_DATA_pt%.1f-%.1f_y0.0-2.4_muPt4.0_centrality0-200_dphiEp_0.00PI_100.00PI.root",fileLoc[ifit].Data(),szAA.Data(),binArr[ib],binArr[ib+1]));}
        else if(ifit==1) {fileIn[ifit][ib]= new TFile(Form("%sfitresults_upsilon_fixParm1_seed2_DoubleCB_%s_DATA_pt%.1f-%.1f_y0.0-2.4_muPt4.0_centrality0-200_dphiEp_0.00PI_100.00PI.root",fileLoc[ifit].Data(),szAA.Data(),binArr[ib],binArr[ib+1]));}
      }
