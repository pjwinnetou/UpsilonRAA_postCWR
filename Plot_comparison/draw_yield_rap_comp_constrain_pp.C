...
int draw_yield_rap_comp_constrain_pp(TString szAA = "PP", int states =1)
{
  setTDRStyle();
...
*/
  ...
  char *Name_Fit[3] = {"All Free","n, #alpha, x, f fixed", "n, #alpha, x fixed, f constrained"};
  ...
  gSig[0]->GetYaxis()->SetTitle("dN/d|y|");
  gSig[0]->GetXaxis()->SetTitle("|y^{#varUpsilon}|");
  gSig[0]->GetYaxis()->CenterTitle();
  gSig[0]->GetXaxis()->CenterTitle();
  gSig[0]->GetXaxis()->SetRangeUser(0,2.4);
  gSig[0]->GetXaxis()->SetLimits(0,2.4);
  ...
  double leg_posy1 = 0.65;
  double leg_posx2 = 0.83;
  double leg_posy2 = 0.75;
...
//  gPad->SetLogy();  // KTO for yield
  ...
  c_nSigs->SaveAs(Form("yield/rap_nSig%ds_%s_constrain.pdf",states,szAA.Data()));
...
//  gPad->SetLogy();  // KTO for yield
  ...
  c_nBkg->SaveAs(Form("yield/rap_nBkg%ds_%s_constrain.pdf",states,szAA.Data()));
