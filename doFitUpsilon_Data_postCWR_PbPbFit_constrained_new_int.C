...
void doFitUpsilon_Data_postCWR_PbPbFit_constrained_new_int(
       int collId = kAADATA,  
       ...
       int FixParm_seed=2  )
{
  ...
  RooRealVar    sigma1s_1("sigma1s_1","width/sigma of the signal gaussian mass PDF",0.04, 0.01, 0.25);
  ...
  RooRealVar alpha1s_1("alpha1s_1","tail shift", 2.6 , 0.5, 4.9);
  ...
  RooRealVar n1s_1("n1s_1","power order", 1.962 , 1.081, 4.762);
...
/*
  cout << "EDM = " << fitRes2->edm() << endl;
  cout << "-log(L) at minimum = " << fitRes2->minNll() << endl;

  // Axxess list of final fit parameter values
  cout << "final value of floating parameters" << endl;
  fitRes2->floatParsFinal().Print("s");

  // Access correlation matrix elements
  cout << "correlation between n and alpha is " << fitRes2->correlation(n1s_1,alpha1s_1) << endl;

  // Extract covariance and correltaion matrix as TMatrixDSym
  const TMatrixDSym& cor = fitRes2->correlationMatrix();
  const TMatrixDSym& cov = fitRes2->covarianceMatrix();
  ...
  TH2* hcorr = fitRes2->correlationHist();
  RooPlot* frame_hist = new RooPlot(*f1s,*x1s,-1,1.,-1,1.);
  fitRes2->plotOn(frame_hist,*f1s,*x1s,"ME12ABHV");
*/
  ...
  TString SaveDir;
  if(FixParm_seed == 1) SaveDir="Int";
  else if(FixParm_seed == 2) SaveDir="Avg";
  
  cout << "chi^{2}/ndf : " << chisq << "/" << ndf << endl;
  TH1D* hChisq = new TH1D("hchisq",";;;",10,0,10);
  hChisq->SetBinContent(1,chisq);
  hChisq->SetBinContent(2,ndf);
  hChisq->SetBinContent(3,chisq/ndf);
  hChisq->SetBinContent(4,theNLL);
  hChisq->GetXaxis()->SetBinLabel(1,"chisquare");
  hChisq->GetXaxis()->SetBinLabel(2,"ndf");
  hChisq->GetXaxis()->SetBinLabel(3,"chisq/ndf");
  hChisq->GetXaxis()->SetBinLabel(4,"nll");

  TFile* outf = new TFile(Form("fitResults/Constrain/testfitresults_upsilon_fixParm%d_seed%d_DoubleCB_%s.root",fixParameters,FixParm_seed,kineLabel.Data()),"recreate");
  outh->Write();
  c1->SaveAs(Form("fitResults/Constrain/testfitresults_upsilon_fixParm%d_seed%d_DoubleCB_%s.pdf",fixParameters,FixParm_seed,kineLabel.Data()));
  c1->Write();
  ws->Write();
  hChisq->Write();
