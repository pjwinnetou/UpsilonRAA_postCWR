...
//  double relsys = 0.1;
  ...
  for (int is=0; is<nState; is++){
    ...
    gRAA[is]->SetName(Form("gRAA_pt_%ds",is));
...
/*        gRAA_sys[2]->SetPoint(0,-10,-10);
  ...
  SetLegendStyle(leg);
  TLegend *leg_up= new TLegend(0.57, 0.50, 0.78, 0.615);
...
//    leg -> AddEntry(gRAA[2]," #Upsilon(3S)","lp");
    ...
    leg_up->Draw("same");
    TLegendEntry *ent1=leg_up->AddEntry("NULL"," #varUpsilon(3S) 95\% CL","");
...
//  globtex->DrawLatex(0.22, sz_init-sz_step*2, "|#eta^{#mu}| < 2.4");
  ...
  cout << "sys_global_y_Lo : " << sys_global_y_Lo << endl;
  cout << "sys_global_y_Hi : " << sys_global_y_Hi << endl;
