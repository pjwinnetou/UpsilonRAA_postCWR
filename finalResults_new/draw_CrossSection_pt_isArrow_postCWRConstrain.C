...
void draw_CrossSection_pt_isArrow_postCWRConstrain(int ppAA=1, bool isArrow=false) //1=pp, 2=AA
{
  setTDRStyle();
...
//  double relsys = 0.1;
  ...
  for (int is=0; is<nState; is++){
    ...
    gCrossSection[is]->SetName(Form("gCrossSection_pt_%s_%d",sz_ppAA.Data(),is));
    gCrossSection_sys[is]->SetName(Form("gCrossSection_pt_%s_%d",sz_ppAA.Data(),is));
  ...
  for (int is=0; is<nState; is++){
    ...
    for (int ipt=0; ipt< npoint[is] ; ipt++) { //bin by bin
      ...
      cout << ipt <<"th bin CrossSectionValue = " << pytmp << endl;
      cout << ipt <<"th bin stat. = " << eytmp << endl;
      //cout << ipt <<"th bin stat. = " << eytmp*exsys_1s[ipt]*2*NumberOfMBColl*4.8*5.607/1000000 << endl;
...
//  }
  ...
  TLegend *leg;
  if (ppAA==1) {leg= new TLegend(0.68, 0.575, 0.85, 0.72); }
  else {leg= new TLegend(0.62, 0.61, 0.83, 0.71);}
  SetLegendStyle(leg);
  ...
  if (isArrow==false) {
    for (int is=0; is<nState; is++){
      leg -> AddEntry(gCrossSection[is],Form(" #varUpsilon(%dS)",is+1),"lp");
    }
    leg->Draw("same");
  }
  else {
    leg -> AddEntry(gCrossSection[0]," #varUpsilon(1S)","lp");
    leg -> AddEntry(gCrossSection[1]," #varUpsilon(2S)","lp");
...
//  globtex->DrawLatex(0.27, sz_init-sz_shift-sz_step*2, "|#eta^{#mu}| < 2.4");
  ...
  c1->SaveAs(Form("Figure2_%s.C",sz_ppAA.Data()));
