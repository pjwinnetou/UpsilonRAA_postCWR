...
void draw_CrossSection_rap_isArrow_postCWRConstrain(int ppAA=1, bool isArrow=false) //1=pp, 2=AA
{
  setTDRStyle();
...
//  }
  ...
  TLegend *leg;
  if (ppAA==1) {leg= new TLegend(0.68, 0.61, 0.85, 0.77); }
  else {leg= new TLegend(0.62, 0.67, 0.83, 0.76);}//leg-> SetNColumns(3);}
  SetLegendStyle(leg);
  TLegend *leg_up= new TLegend(0.62, 0.58, 0.83, 0.67);
  SetLegendStyle(leg_up);

  TArrow *arrLeg = new TArrow(1.39,1.4275,1.39,2.09,0.021,"<-|");
...
//    ent->SetFillColorAlpha(kGreen-6,0.5);
    ...
    ent=leg_up->AddEntry("ent"," #varUpsilon(3S) 95\% CL","");
...
//  globtex->DrawLatex(0.27, sz_init-sz_shift-sz_step*2, "|#eta^{#mu}| < 2.4");
  ...
  c1->SaveAs(Form("Figure3_%s.C",sz_ppAA.Data()));
