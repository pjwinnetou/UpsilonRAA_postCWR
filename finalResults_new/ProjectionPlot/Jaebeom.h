#ifndef COMMONOPT_Jaebeom_H
#define COMMONOPT_Jaebeom_H
...
void SetGraphStyle(TGraph* gr, Int_t c, Int_t m) {
  ...
  Int_t colorArr[] = { kBlack, kGray+3, kGreen+2, kViolet-6, kBlack };
...
void SetGraphStyleSys(TGraph* gr, Int_t c) {
  //Int_t colorArr[] = { kPink-6, kBlue-2, kGreen+3, kViolet-6, kBlack };
  Int_t colorArr[] = { kBlack, kGray+3, kGreen+2, kViolet-6, kBlack };
  Int_t colorArrSys[] = { kBlack, kGray+3, kGreen-10, kMagenta-10, kGray};
  gr-> SetLineColor(colorArr[c]);
  gr-> SetFillColorAlpha(colorArrSys[c],0.0);
  gr-> SetLineWidth(2);
}
