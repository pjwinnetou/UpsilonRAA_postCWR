...
void getEfficiencyUpsilon_new_var(int state = 1, bool useDataWeight=true, 
  ...
  TFile *weightFileAA; 
  if(state==1){
    weightFileAA = new TFile("/home/deathold/work/CMS/analysis/Upsilon_RAA/upsilonRAA_postCWR/UpsilonRAA_postCWR/CompareDataMC/WeightedFcN_fit/ratioDataMC_AA_DATA_1s_20180418.root","read");
    weightFilePP = new TFile("/home/deathold/work/CMS/analysis/Upsilon_RAA/upsilonRAA_postCWR/UpsilonRAA_postCWR/CompareDataMC/WeightedFcN_fit/ratioDataMC_PP_DATA_1s_20180418.root","read");
  }
  else if(state!=1){
    weightFileAA = new TFile("/home/deathold/work/CMS/analysis/Upsilon_RAA/upsilonRAA_postCWR/UpsilonRAA_postCWR/CompareDataMC/WeightedFcN_fit/ratioDataMC_AA_DATA_2s_20180418.root","read");
    weightFilePP = new TFile("/home/deathold/work/CMS/analysis/Upsilon_RAA/upsilonRAA_postCWR/UpsilonRAA_postCWR/CompareDataMC/WeightedFcN_fit/ratioDataMC_PP_DATA_2s_20180418.root","read");
  }

  TF1 *funcPP_nom = (TF1*) weightFilePP->Get("dataMC_Ratio_norm");
  TF1 *funcAA_nom = (TF1*) weightFileAA->Get("dataMC_Ratio_norm");
  TF1 *funcPP_Ap = (TF1*) weightFilePP->Get("dataMC_Ratio_Ap");
  TF1 *funcPP_Am = (TF1*) weightFilePP->Get("dataMC_Ratio_Am");
  TF1 *funcPP_Bp = (TF1*) weightFilePP->Get("dataMC_Ratio_Bp");
  TF1 *funcPP_Bm = (TF1*) weightFilePP->Get("dataMC_Ratio_Bm");
  TF1 *funcAA_Ap = (TF1*) weightFileAA->Get("dataMC_Ratio_Ap");
  TF1 *funcAA_Am = (TF1*) weightFileAA->Get("dataMC_Ratio_Am");
  TF1 *funcAA_Bp = (TF1*) weightFileAA->Get("dataMC_Ratio_Bp");
  TF1 *funcAA_Bm = (TF1*) weightFileAA->Get("dataMC_Ratio_Bm");
  ...
  if(state==1){
   funcPP_Cp = (TF1*) weightFilePP->Get("dataMC_Ratio_Cp");
   funcPP_Cm = (TF1*) weightFilePP->Get("dataMC_Ratio_Cm");
   funcPP_Dp = (TF1*) weightFilePP->Get("dataMC_Ratio_Dp");
   funcPP_Dm = (TF1*) weightFilePP->Get("dataMC_Ratio_Dm");
   funcAA_Cp = (TF1*) weightFileAA->Get("dataMC_Ratio_Cp");
   funcAA_Cm = (TF1*) weightFileAA->Get("dataMC_Ratio_Cm");
   funcAA_Dp = (TF1*) weightFileAA->Get("dataMC_Ratio_Dp");
   funcAA_Dm = (TF1*) weightFileAA->Get("dataMC_Ratio_Dm");
  }

  int nVaria = 2;
  if(state==1) nVaria=4;

  double parm_AA_nom[10]; 
  double parm_PP_nom[10]; 

  double parm_AA_up[10]; 
  double parm_PP_up[10];
  double parm_AA_do[10]; 
  double parm_PP_do[10];


  for(int i=0; i<nVaria; i++){
    parm_PP_nom[i] = funcPP_nom->GetParameter(i);
    parm_AA_nom[i] = funcAA_nom->GetParameter(i);
  }

  parm_PP_up[0] = funcPP_Ap->GetParameter(0);
  parm_PP_up[1] = funcPP_Bp->GetParameter(1);
  parm_PP_do[0] = funcPP_Am->GetParameter(0);
  parm_PP_do[1] = funcPP_Bm->GetParameter(1);
  parm_AA_up[0] = funcAA_Ap->GetParameter(0);
  parm_AA_up[1] = funcAA_Bp->GetParameter(1);
  parm_AA_do[0] = funcAA_Am->GetParameter(0);
  parm_AA_do[1] = funcAA_Bm->GetParameter(1);
  
  if(state==1){
    parm_PP_up[2] = funcPP_Cp->GetParameter(2);
    parm_PP_up[3] = funcPP_Dp->GetParameter(3);
    parm_PP_do[2] = funcPP_Cm->GetParameter(2);
    parm_PP_do[3] = funcPP_Dm->GetParameter(3);
    parm_AA_up[2] = funcAA_Cp->GetParameter(2);
    parm_AA_up[3] = funcAA_Dp->GetParameter(3);
    parm_AA_do[2] = funcAA_Cm->GetParameter(2);
    parm_AA_do[3] = funcAA_Dm->GetParameter(3);
  }
  ...
  if ( state == 1) { 
    ...
    fWgtAA -> SetParameters(parm_AA_nom[0],parm_AA_nom[1],parm_AA_nom[2],parm_AA_do[3] );
    fWgtPP -> SetParameters(parm_PP_nom[0],parm_PP_nom[1],parm_PP_nom[2],parm_PP_do[3] );
  }
  else if ( state == 2 ) { 
    ...
    fWgtAA -> SetParameters(parm_AA_nom[0],parm_AA_nom[1] );
    fWgtPP -> SetParameters(parm_PP_nom[0],parm_PP_nom[1] );
  }
  else if ( state == 3 ) { 
  ...
  cout << "*** parameter confirm *** " << endl;  
  cout << "fWgtPP Par 0 : " << fWgtPP->GetParameter(0)  << endl;  
  cout << "fWgtAA Par 0 : " << fWgtAA->GetParameter(0)  << endl;  
  cout << "fWgtPP Par 1 : " << fWgtPP->GetParameter(3)  << endl;  
  cout << "fWgtAA Par 1 : " << fWgtAA->GetParameter(3)  << endl;  
  ...
  TFile* fout = new TFile(Form("%s/VarDdo_efficiency_ups%ds_useDataPtWeight%d_tnp_trgId%d_trkId%d_muId%d_staId%d.root",outputDirName.Data(), state,useDataWeight,trgIdx, trkIdx, muIdx, staIdx),"recreate");

  c_eff_pt->SaveAs(Form("%s/VarDdo_eff_vs_pt_%ds_useDataPtWeight%d_tnp_trgId%d_trkId%d_muId%d_staId%d.pdf", outputDirName.Data(), state, useDataWeight, trgIdx, trkIdx, muIdx, staIdx ));
  c_eff_rap->SaveAs(Form("%s/VarDdo_eff_vs_rap_%ds_useDataPtWeight%d_tnp_trgId%d_trkId%d_muId%d_staId%d.pdf", outputDirName.Data(), state, useDataWeight, trgIdx, trkIdx, muIdx, staIdx ));
  c_eff_cent->SaveAs(Form("%s/VarDdo_eff_vs_cent_%ds_useDataPtWeight%d_tnp_trgId%d_trkId%d_muId%d_staId%d.pdf", outputDirName.Data(), state, useDataWeight, trgIdx, trkIdx, muIdx, staIdx ));
