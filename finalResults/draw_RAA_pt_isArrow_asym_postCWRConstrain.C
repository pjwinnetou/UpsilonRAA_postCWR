...
//  double relsys = 0.1;
  ...
  //// legend
  //// axis et. al
  gRAA_sys[0]->GetXaxis()->SetTitle("p_{T}^{#varUpsilon} (GeV)");
...
//  globtex->DrawLatex(0.22, sz_init, "p_{T}^{#mu#mu} < 30 GeV/c");
  globtex->DrawLatex(0.22, sz_init-sz_step, "|y^{#varUpsilon}| < 2.4");
//  globtex->DrawLatex(0.22, sz_init-sz_step*2, "|#eta^{#mu}| < 2.4");
  ...
  CMS_lumi_raaCent( c1, iPeriod, iPos );
	...
	c1->Update();
  c1->SaveAs(Form("RAA_vs_pt_isArrow%d_asym_postCWRConstrain.pdf",(int)isArrow));
  c1->SaveAs(Form("RAA_vs_pt_isArrow%d_asym_postCWRConstrain.png",(int)isArrow));
