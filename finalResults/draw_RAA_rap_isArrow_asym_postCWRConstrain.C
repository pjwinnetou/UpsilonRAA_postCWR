...
//  double relsys = 0.1;
  ...
  //// axis et. al
  gRAA_sys[0]->GetXaxis()->SetTitle("|y^{#varUpsilon}|");
...
//  globtex->DrawLatex(0.22, sz_init, "p_{T}^{#mu} > 4 GeV/c");
  globtex->DrawLatex(0.22, sz_init-sz_step, "p_{T}^{#varUpsilon} < 30 GeV");
...
//  globtex->DrawLatex(0.22, sz_init-sz_step*2, "|#eta^{#mu}| < 2.4");
  ...
  CMS_lumi_raaCent( c1, iPeriod, iPos );
	...
	c1->Update();
  c1->SaveAs(Form("RAA_vs_rap_isArrow%d_asym_postCWRConstrain.pdf",(int)isArrow));
  c1->SaveAs(Form("RAA_vs_rap_isArrow%d_asym_postCWRConstrain.png",(int)isArrow));
