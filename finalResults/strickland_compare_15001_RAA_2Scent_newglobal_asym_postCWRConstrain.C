...
void strickland_compare_15001_RAA_2Scent_newglobal_asym_postCWRConstrain(int istate=2) //1 or 2 (1S or 2S)
{
  setTDRStyle();
...
*/
 ...
 // TLegend *leg= new TLegend(0.6, 0.656, 1.0, 0.786);
  ...
  TLegend *leg= new TLegend(0.25, 0.5045, 0.68, 0.6665);
  SetLegendStyle(leg);
...
//  globtex->DrawLatex(0.46+0.04, sz_init+0.002, "|#eta|^{#mu} < 2.4");
  ...
  for(int i=0;i<3;i++)
  {
    gRAA_1S_strickland[i] = (TGraphErrors*) fstrickland_2760-> Get(Form("RAA_strick_nPart_%dS_%d",istate,i));
    gRAA_2S_strickland[i] = (TGraphErrors*) fstrickland_5023-> Get(Form("RAA_strick_nPart_%dS_%d",istate,i));
    gRAA_1S_strickland[i] -> SetLineWidth(4.);
    gRAA_2S_strickland[i] -> SetLineWidth(4.0);
  }
  gRAA_1S_strickland[0]->SetLineColor(kGray+3);
  gRAA_1S_strickland[1]->SetLineColor(kGray+3);
  gRAA_1S_strickland[2]->SetLineColor(kGray+3);
  gRAA_1S_strickland[0]->SetLineStyle(3);
  gRAA_1S_strickland[1]->SetLineStyle(1);
  gRAA_1S_strickland[2]->SetLineStyle(8);
  
  gRAA_2S_strickland[0]->SetLineColor(kBlue);
  gRAA_2S_strickland[1]->SetLineColor(kBlue);
  gRAA_2S_strickland[2]->SetLineColor(kBlue);
  ...
  t3->SetLineStyle(8);
  t3->SetLineWidth(2);
  t3->SetLineColor(kGray+3);
  t3->Draw("same");

   TLine* t33 = new TLine(line_x_start,line_y-line_y_diff*2-line_y_diff_in,line_x_end,line_y-line_y_diff*2-line_y_diff_in);
   t33->SetLineStyle(8);
   t33->SetLineWidth(2);
   t33->SetLineColor(kBlue);
   t33->Draw("same");

  
  drawText2("4#pi #eta/s=1", line_x_end+7, line_y-0.031, 19);
  drawText2("4#pi #eta/s=2", line_x_end+7, line_y-line_y_diff*1-0.033, 19);
  drawText2("4#pi #eta/s=3", line_x_end+7, line_y-line_y_diff*2-0.035, 19);
...
//  gRAA_int_sys[0]->SetMaximum(1.3);
  ...
  c1->SaveAs(Form("plots/Strickland_%dS_comp15001_RAA_vs_cent_newglobal_asym_postCWRConstrain.pdf",istate));
  c1->SaveAs(Form("plots/Strickland_%dS_comp15001_RAA_vs_cent_newglobal_asym_postCWRConstrain.png",istate));
