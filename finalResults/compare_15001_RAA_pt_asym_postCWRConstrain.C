...
void compare_15001_RAA_pt_asym_postCWRConstrain(int istate=1) //1 or 2 (1S or 2S)
{
  setTDRStyle();
...
//  globtex->DrawLatex(0.22, sz_init-sz_step*2, "|#eta^{#mu}| < 2.4");
  ...
  CMS_lumi_raaCent( c1, iPeriod, iPos );
	...
	c1->Update();
  c1->SaveAs(Form("plots/%dS_comp15001_RAA_vs_pt_asym_Constrain.pdf",istate));
  c1->SaveAs(Form("plots/%dS_comp15001_RAA_vs_pt_asym_Constrain.png",istate));
