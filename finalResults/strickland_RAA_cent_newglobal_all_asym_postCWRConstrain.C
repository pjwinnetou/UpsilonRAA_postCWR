...
//  globtex->DrawLatex(0.22+0.04, sz_init, "p_{T}^{#mu} > 4 GeV/c");
  globtex->DrawLatex(0.22+0.1, sz_init, "p_{T}^{#mu#mu} < 30 GeV");
...
//  CMS_lumi( c1, iPeriod, iPos );
  ...
  globtex->DrawLatex(0.5*(1-0.032*600/xlonger), sz_init-sz_step*2-sz_allign, "0-100%");
	...
	c1->Update();
  c1->SaveAs(Form("Strickland_RAA_vs_cent_isArrow%d_all_newglobal_asym_postCWRConstrain.png",(int)isArrow));

  c1->SaveAs(Form("Strickland_RAA_vs_cent_isArrow%d_all_newglobal_asym_postCWRConstrain.pdf",(int)isArrow));
/*
	///////////////////////////////////////////////////////////////////
	//// save as a root file
