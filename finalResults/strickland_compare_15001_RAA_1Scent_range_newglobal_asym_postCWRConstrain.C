...
void strickland_compare_15001_RAA_1Scent_range_newglobal_asym_postCWRConstrain(int istate=1) //1 or 2 (1S or 2S)
{
  setTDRStyle();
...
*/
 ...
 // TLegend *leg= new TLegend(0.6, 0.656, 1.0, 0.786);
  ...
  TLegend *leg= new TLegend(0.25, 0.182, 0.68, 0.356);
  SetLegendStyle(leg);
...
//  globtex->DrawLatex(0.46+0.04, sz_init+0.002, "|#eta|^{#mu} < 2.4");
  ...
  gRAA_2S_strickland[0]->SetLineColor(kRed+1);
  gRAA_2S_strickland[1]->SetLineColor(kRed+1);
  gRAA_2S_strickland[2]->SetLineColor(kRed+1);
 ...
 // TLegend *leg_strick= new TLegend(0.25, 0.656, 0.48, 0.786);
  ...
  t3->Draw("same");
   ...
   t33->SetLineStyle(8);
   t33->SetLineWidth(2);
   t33->SetLineColor(kRed+1);
   t33->Draw("same");

  
  drawText2("4#pi #eta/s=1", line_x_end+7, line_y-0.024, 19);
  drawText2("4#pi #eta/s=2", line_x_end+7, line_y-line_y_diff*1-0.026, 19);
  drawText2("4#pi #eta/s=3", line_x_end+7, line_y-line_y_diff*2-0.028, 19);
  ...
  sys_global_y = TMath::Sqrt(sys_global_y*sys_global_y+relsys_ppInt1S*relsys_ppInt1S);
...
//  gRAA_int_sys[0]->SetMaximum(1.3);
  ...
  c1->SaveAs(Form("plots/Strickland_%dS_comp15001_RAA_vs_cent_range_newglobal_asym_postCWRConstrain.pdf",istate));
  c1->SaveAs(Form("plots/Strickland_%dS_comp15001_RAA_vs_cent_range_newglobal_asym_postCWRConstrain.png",istate));
